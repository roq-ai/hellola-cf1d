...
import { getMarketplaceById, updateMarketplaceById } from 'apiSdk/marketplaces';
...
function MarketplaceEditPage() {
  ...
  const handleSubmit = async (values: MarketplaceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateMarketplaceById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<MarketplaceInterface>({
    initialValues: data,

...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['seller-owner', 'buyer-owner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<MarketplaceInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/marketplaces` : null),
    () => getMarketplaces({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Marketplace',
      path: isTenantUser ? `/marketplaces/view/${data?.[0]?.id}` : '/marketplaces',
      entity: 'marketplace',
      service: AccessServiceEnum.PROJECT,
      icon: FiShoppingBag,
    },
    { name: 'Auction', path: '/auctions', entity: 'auction', service: AccessServiceEnum.PROJECT, icon: FiClock },
    { name: 'Bid', path: '/bids', entity: 'bid', service: AccessServiceEnum.PROJECT, icon: FiDollarSign },
